...
function createPlayerXpCard(user, guild, rank, xp, level, xpRemaining, lvlXp, playerColor)
	...
	local file = io.popen(("python3 utils/create_xp.py %i %i %i %i %i 3 5 64 64 65 53 320 20 %x FFFFFF '%s#%s' '%s' ./ressources/xp_bg.png xp_cards/xp_card_%s_%s.png 2>&1"):format(rank, level, xpRemaining, lvlXp, xp, playerColor, user.name:gsub("'", "\\'"), user.discriminator, user.avatarURL, guild.id, user.id))
	local lines = arrayFromIt(file:lines())
	local success, _, status = file:close()

	if not success then
		return nil, "Command '"..("python3 utils/create_xp.py %i %i %i %i %i 3 5 64 64 65 53 320 20 %x FFFFFF '%s#%s' '%s' ./ressources/xp_bg.png xp_cards/xp_card_%s_%s.png 2>&1"):format(rank, level, xpRemaining, lvlXp, xp, playerColor, user.name, user.discriminator, user.avatarURL, guild.id, user.id).."' exit with error code "..tostring(status)..":\n"..table.concat(lines, "\n")
	else
